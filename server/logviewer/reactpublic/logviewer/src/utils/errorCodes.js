...
export function lookupErrorCode(input) {
  ...
  if (!isNaN(dec)) {
    const u32 = dec >>> 0;
    const hexFromDec = '0x' + u32.toString(16).toUpperCase().padStart(8, '0');
...
export function findErrorCodesInText(text) {
  if (!text) return [];
  const results = [];
  const seen = new Set();
  const hexRe = /0x[0-9A-Fa-f]{4,8}/gi;
  let m;
  while ((m = hexRe.exec(text)) !== null) {
    const code = m[0];
    if (seen.has(code.toLowerCase())) continue;
    seen.add(code.toLowerCase());
    const desc = lookupErrorCode(code);
    if (desc) results.push({ code, description: desc });
  }
  const decRe = /\b(1[56]\d{2}|1[0-4]\d{2}|30[01]\d)\b/g;
