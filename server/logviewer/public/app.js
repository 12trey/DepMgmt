...
// ── Ansible Playbook Log quick-link ────────────────────────────────────────
function openAnsibleLog() {
  socket.emit('unwatch');
  tailPaused = false;
  if (pauseBtn) { pauseBtn.textContent = 'Pause'; pauseBtn.classList.remove('active'); }
  ...
  currentFilePath = null;
  resetViewer();
  currentFileEl.textContent = 'Ansible Playbook Log (live)';
  currentFileEl.title = '/tmp/ansible_cmtrace.log';
  document.title = 'Ansible Playbook Log - Log Viewer';
  showLogArea();
  logScrollInner.innerHTML = '<div class="loading">Loading Ansible log\u2026</div>';
  statusWatching.textContent = '\uD83D\uDD74 Live: Ansible Playbook Log';

  // Mark quick-link as active, clear file-list selection
  document.querySelectorAll('#quick-links-list li').forEach(function(li) { li.classList.remove('active'); });
  document.querySelectorAll('#file-list li').forEach(function(li) { li.classList.remove('selected'); });
  var link = $('ansible-log-link');
...
(function() {
  var link = $('ansible-log-link');
  if (link) link.onclick = openAnsibleLog;
})();
...
socket.on('log:lines', function(data) {
  ...
  // Clear any loading placeholder on first batch
  var loading = logScrollInner.querySelector('.loading');
  if (loading) logScrollInner.innerHTML = '';
