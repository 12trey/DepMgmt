...
export default function EditPackage() {
    ...
    getPackage(appName, version)
      .then((pkg) => {
        setForm({
          ...
          version: pkg.version || '',
          vendor: pkg.vendor || '',
          architecture: pkg.architecture || 'x64',
          psadtVersion: pkg.psadtVersion || 'v3',
          installCommand: pkg.installCommand || '',
          uninstallCommand: pkg.uninstallCommand || '',
          repairCommand: pkg.repairCommand || '',
          closeApps: pkg.closeApps || '',
          defaultMode: pkg.defaultMode || 'Silent',
          detection: pkg.detection || { type: 'file', path: '', valueName: '', productCode: '', script: '' },
          preInstallSteps: pkg.preInstallSteps || [],
          postInstallSteps: pkg.postInstallSteps || [],
  ...
  const handleSubmit = async (e) => {
    ...
    setError('');
    try {
      await updatePackage(appName, version, form);
      navigate(`/packages/${appName}/${version}`);
    } catch (err) {
  ...
  return (
    <div className="max-w-3xl">
      <h1 className="text-2xl font-bold mb-6">Edit Package — {appName} v{version}</h1>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <Section title="Commands">
          <Textarea
            ...
            placeholder={form.psadtVersion === 'v4'
              ? 'e.g. Start-ADTMsiProcess -Action Install -FilePath "$dirFiles\\setup.msi"'
        ...
        <div className="flex gap-3">
          ...
          <button type="button" onClick={() => navigate(`/packages/${appName}/${version}`)} className="btn-secondary">
            Cancel
          </button>
        </div>
      </form>
...
function Textarea({ label, className = '', ...props }) {
  return (
    <label className={`block ${className}`}>
      <span className="text-sm font-medium text-gray-700">{label}</span>
      <textarea className="input mt-1 font-mono text-sm h-20" {...props} onChange={(e) => props.onChange(e.target.value)} />
    </label>
  );
}
